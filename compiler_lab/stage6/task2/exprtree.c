...
struct tnode* makeFuncDefNode(char *retType,char *name,struct tnode* body,struct tnode* plist){
   ...
   if(strcmp(retType,gfunc->type->name)==0){
     struct tnode* temp = (struct tnode*)malloc(sizeof(struct tnode));
    ...
    printf("Type Mismatch 5 %s %s\n",gfunc->type->name,retType);
    exit(1);
   }
...
struct tnode* makeFuncCallNode(char *funcname,struct tnode* argList){
       ...
       if(strcmp("alloc",funcname)==0 || strcmp("initialize",funcname)==0 || strcmp("free",funcname)==0){
...
struct tnode* makeRetNode(struct tnode *expr){
     ...
     if(strcmp(expr->type->name,temp->type->name)==0){
      struct tnode *newnode = (struct tnode*)malloc(sizeof(struct tnode));
       newnode->left = expr;
       newnode->Gentry = temp;
       newnode->Lentry = test;
...
struct tnode* makeUserDefinedNode(struct tnode *node){
     ...
     while(temp1->cond!=NULL){
          /*if(temp->cond!=NULL){
            if(strcmp(temp->type->name,"int")==0){
              printf("Error\n");
              exit(1);
            }
          }*/
...
int dataTypeCheck(struct tnode* l,struct tnode* r){
    ...
    if(r->varname!=NULL){
       if(strcmp(r->varname,"null")==0 || strcmp(r->varname,"alloc")==0 || strcmp(r->varname,"initialize")==0 || strcmp(r->varname,"free")==0 ){
      return 1;
      }  
    }
    while(l->cond!=NULL){
