...
struct Classtable *CInstall(char *name,char *parent_name){
     ...
     /*if(inherit==1){
      struct Classtable *parent = CLookup(parent_name);
      struct Fieldlist *flist = parent->Memberfield;
      while(flist!=NULL){
        struct Fieldlist *fchildlist = newnode->Memberfield;
        struct Fieldlist *node = (struct Fieldlist*)malloc(sizeof(struct Fieldlist));
        node->name = malloc(sizeof(char));
        strcpy(node->name,flist->name);
        node->next = NULL;
        node->type = flist->type;
        node->Ctype = flist;
      }
     }*/
