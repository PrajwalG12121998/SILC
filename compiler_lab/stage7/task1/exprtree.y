...
ClassDefBlock : CLASS ClassDefList ENDCLASS  {Cptr = NULL;}
              |
              ;
...
ClassDef      : Cname '{' DECL Fieldlists MethodDecl ENDDECL MethodDefns '}' { addField_and_Method_count(Cptr,classFieldcount,Methodcount); }
              ;
...
MainBlock : Main '(' ')' '{' LDeclBlock Body '}' { $$ = makeFuncDefNode("int","main",$6,NULL,NULL);}
          ;
...
FieldFunction : SELF '.' ID '(' ArgList ')' {$1 = makeVariableNode(VAR_,"self",NULL); $1->cond = $3; $3->cond=NULL;  $$ = $1;}
              | ID '.' ID '(' ArgList ')'  {$$=makeMethodCallNode($1,$3->varname,$5);}
              | ID '.' FIELD '(' ArgList ')'  {$1->cond = $3; $$=$1;}

Retstmt : RETURN expr SEMICOLON {printf("I m in ret node %s\n",$2->type->name);$$ = makeRetNode($2);}
        ;
