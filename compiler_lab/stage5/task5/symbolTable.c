...
void GInstall(char *name,int type,int size1,int size2,struct tnode *node){
...
void LInstall(char *funcname,int type,char *varname,int binding){
...
void printSymbolTable(){
  ...
  while(test!=NULL){
    ...
    if(test->lentry!=NULL){
      ...
      while(Llist!=NULL){
        printf("%s %d %d\n",Llist->name,Llist->type,Llist->binding);
        Llist = Llist->next;
      }
    }
    test = test->next;
