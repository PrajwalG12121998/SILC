...
int codeGen(struct tnode* t,FILE *target_file){
    ...
    switch(t->nodetype){
      ...
      case FCALL_ :{ printf("INside fcall\n"); 
                     push_registers(target_file);
                     freeAllReg();
                     struct tnode* arg = t->argList;
                     printf("%s %d\n",t->varname,arg->nodetype);
                     while(arg!=NULL){
                       printf("arg not null\n");
                       p = codeGen(arg,target_file);
                       printf("%d %d\n",p,RegNo);
                       fprintf(target_file, "PUSH R%d\n",p);
                       arg = arg->argList;
                     }
      ...
      case FUNC_ :{ 
                   ...
                   if(strcmp(t->varname,"main")==0){  
                    ...
                    struct localTable *table = t->Lentry;
                    struct Lsymbol *test = table->lentry;
  
                    while(test!=NULL){//pushing local var
                      if(test->binding>0)
                         fprintf(target_file, "PUSH R0\n");
                      test = test->next;
                    }
      ...
      case BODY_ :{  printf("INside body\n");
                     codeGen(t->left,target_file);
                     codeGen(t->right,target_file);
                     break;
                  }
      case RET_ :{  printf("INside ret\n");
                    p = getReg();
                    if(strcmp(t->Gentry->name,"main")!=0){
                      ...
                      //calculate return expression and store in BP-2
                      fprintf(target_file, "MOV R%d,BP\n",p);
