...
Cname         : ID              {Cptr = CInstall($1->varname,NULL); classFieldcount=0; Methodcount=0;parentName=NULL;}         
              | ID EXTENDS ID   {Cptr = CInstall($1->varname,$3->varname); classFieldcount=0; Methodcount=0;
                                Child_Class_FInstall(Cptr,$3->varname);parentName = strdup($3->varname);}
              ;
...
LDeclBlock : DECL LDecList ENDDECL {LInstall(currFunc,"NULL","self",getArgOffset());}
           | DECL ENDDECL {LInstall(currFunc,"NULL","self",getArgOffset());}
           ;
