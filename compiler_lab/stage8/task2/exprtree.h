...
char *currFunc,*currUserDef,*currType,*currType1,*currType2,*parentName;
int classFieldcount,Methodcount;
struct Classtable *Cptr;

typedef struct tnode{
     int val; // value of a number for NUM nodes.
     int type1;  //type of variable
     char *str_val; //to store string 
     char *varname;  //name of a variable for ID nodes
     int nodetype;  //information about non-leaf node - read/write/connector/+/* etc
     struct TypeTable *type;
     struct tnode *left,*right,*argList,*middle; //left and right branches
