  ...
  printf("%d\n",check);
   if(l->Ctype!=NULL && r->Ctype!=NULL){
    if(strcmp(l->Ctype->name,r->Ctype->name)==0){
      ...
      //TODO
      while(rclass->Parentptr!=NULL){
        if(strcmp(rclass->Parentptr->name,l->Ctype->name)==0){
        check = 1;
        break;  
        }
        rclass = rclass->Parentptr;  
      }  
...
struct tnode* makeMethodCallNode(struct tnode *classVar,char *methodname,struct tnode *argList){
   ...
   while(paramlist!=NULL && alist!=NULL){
    printf("%s %s\n",paramlist->type->name,alist->type->name);
...
int codeGen(struct tnode* t,FILE *target_file){
    ...
    switch(t->nodetype){
    	case READ_ :{ q = getReg(); r = getReg(); int r1;printf("I am in read\n");
                  if(t->left->nodetype==USERDEF_){
                     printf("in Userdef\n");
                     r1 = getUserDefinedAddress(t->left,target_file); //changed in stage8

                  }
                  else{printf("Not in Userdef\n");
      ...
      case PRNT_ : {  p = getReg();
    	...
    	case ASSIGN_ :{ //printf("I am in assign %d\n",t->right->nodetype);
                    ...
                    //fprintf(target_file, "MOV R%d, R%d\n",p,q);
    		            fprintf(target_file, "MOV [R%d], R%d\n",p,q);
                    //printf("before assign %s\n",t->right->varname);

                    if(t->left->Ctype!=NULL && t->right->Ctype!=NULL && t->right->nodetype==VAR_){
                       printf("%s\n",t->right->varname );
                       struct Gsymbol *gtemp = t->right->Gentry; 
                       fprintf(target_file, "MOV R%d,[%d]\n",q,gtemp->binding+1);
                       fprintf(target_file, "ADD R%d,1\n",p );
                       fprintf(target_file, "MOV [R%d],R%d\n",p,q); 
                      }

                    if(t->right->nodetype==FCALL_ && t->right->varname!=NULL && strcmp(t->right->varname,"new")==0){
      ...
      case MCALL_ : {  //push_registers
                     ...
                     if(strcmp(t->right->varname,"self")==0){
                       ...
                       if(gtemp!=NULL){
                        ...
                        //printf("In local mcall %s\n",t->Ctype->name);
                         ...
                         q = getMemoryAddress("self",target_file);
                         fprintf(target_file, "MOV R%d,[R%d]\n",q,q );
                         fprintf(target_file, "ADD R%d,%d\n",q,ftemp->fieldIndex);
                         fprintf(target_file, "MOV R%d,[R%d]\n",p,q );
                         fprintf(target_file, "PUSH R%d\n",p);
                     ...
                     while(temp!=NULL){
                       //printf("%d %s\n",temp->nodetype,temp->varname);
                       printf("Inside MCALL Arguments %s\n",temp);
                     ...
                     if(gtemp!=NULL){
                     ...
                     else if(strcmp(t->right->varname,"self")==0){
                       q = getMemoryAddress(t->right,target_file);
                       fprintf(target_file, "MOV R%d,[R%d]\n",p,q);
                       fprintf(target_file, "ADD R%d,1\n",q);
                       fprintf(target_file, "MOV R%d,[R%d]\n",p,q);
                       ...
                       fprintf(target_file, "CALL R%d\n",p);
                        ...
                        if(mlist==NULL){
                           ...
                           struct Classtable *cvar = CLookup(ftemp->Ctype->name);

                           mlist = Class_MLookup(cvar,t->varname);
                           //printf("Inside Field %s %s %d\n",mlist,t->Ctype->name,ftemp->fieldIndex);    
                        }
                        q = getMemoryAddress("self",target_file);
                        fprintf(target_file, "MOV R%d,[R%d]\n",q,q );
                        fprintf(target_file, "ADD R%d,%d\n",q,ftemp->fieldIndex+1);
                        fprintf(target_file, "MOV R%d,[R%d]\n",q,q);
                        fprintf(target_file, "ADD R%d,%d\n",q,mlist->Funcposition);
                        fprintf(target_file, "MOV R%d,[R%d]\n",q,q);
                        fprintf(target_file, "CALL R%d\n",q);
                     }
