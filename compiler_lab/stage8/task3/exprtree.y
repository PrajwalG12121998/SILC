...
ClassDefBlock : CLASS ClassDefList ENDCLASS  { $$ = $2; Cptr = NULL; staticAddress=4095 + 8*classIndex;}
              |                              {Cptr=NULL;}
              ;
...
Cname         : ID              {Cptr = CInstall($1->varname,NULL); classFieldcount=0; Methodcount=0;parentName=NULL;}         
              | ID EXTENDS ID   {Cptr = CInstall($1->varname,$3->varname); classFieldcount=0; Methodcount=0;
                                Child_Class_FInstall(Cptr,$3->varname);parentName = strdup($3->varname);Child_Class_MInstall(Cptr,parentName);}
              ;

Fieldlists  : Fieldlists Fld    {classFieldcount++;Class_FInstall(Cptr,"NULL","vptr");}
            |
            ;
...
MDecl      : ID ID '(' ParamList ')' SEMICOLON  {Class_MInstall(Cptr,$2->varname,$1->varname,$4);/*LInstall($2->varname,"null","self",getArgOffset());*/} 
...
Gid        :   ID                      { if(CLookup(currType)!=NULL){GInstall($1->varname,currType, 1, 2,NULL);}else{GInstall($1->varname,currType, 1, 1,NULL);}}
...
LDeclBlock : DECL LDecList ENDDECL {LInstall(currFunc,"NULL","self_ptr",getArgOffset());LInstall(currFunc,"NULL","self",getArgOffset());}
           | DECL ENDDECL {LInstall(currFunc,"NULL","self_ptr",getArgOffset());LInstall(currFunc,"NULL","self",getArgOffset());}
           ;
...
FIELD : SELF '.' ID  { $1 = makeVariableNode(VAR_,"self",NULL); $1->cond = $3; $3->cond=NULL;  $$ = $1; 
      ...
      | ID '.' ID    {if($1->Ctype!=NULL){printf("ERROR\n");exit(1);}$1->cond = $3; $3->cond=NULL; $$=$1;printf("Hi in field 4\n");}
      ;
...
ArgList : ArgList ',' expr  { $3->middle = $1;$$=$3;printf("In arglist\n");}
        | expr    {$1->middle=NULL;$$ = $1;printf("In arglist 1\n");}
        |         {$$=NULL;}
        ;
